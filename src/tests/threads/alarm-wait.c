...
static struct semaphore hello_sema;
static void
hello_pintos_start (void *aux UNUSED) {
  hello_pintos ();
  sema_up (&hello_sema);
}

void
test_hello_pintos (void) {
  sema_init (&hello_sema, 0);
  thread_create ("hello", PRI_DEFAULT, hello_pintos_start, NULL);
  sema_down (&hello_sema);
}
