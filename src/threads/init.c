...
#endif
void printList(struct list* output_list) {
  struct list_elem *e;
  for(e = list_begin(output_list) ; e != list_end(output_list) ; e = list_next(e)) {
    struct sortNumber *num = list_entry (e, struct sortNumber, elem);
    printf("%d ", num->number);
  }
  printf("\n");
}
void createRandomNum() {
  ...
  int8_t* randomNum = (int8_t*)malloc(sizeof(int8_t) * 100);
  random_bytes(randomNum, 100);
  int i = 0;
  while(i < 100) {
    struct sortNumber *sortNum = (struct sortNumber*)malloc(sizeof(struct sortNumber));
    sortNum->number =  randomNum[i];
    list_push_back(&number_list, &(sortNum->elem));
    ++i;
  }
...
void sortRandomNum() {
  ...
  list_sort(&number_list,&list_less_func,NULL);
  intr_set_level(old_level);
}
void outputResult() {
  enum intr_level old_level;
...
void hello_pintos (void) {
  ...
  printf("hello, lujianye!\n");
  tid_t thread1 = thread_create("thread1", 60, &createRandomNum,NULL);
  tid_t thread2 = thread_create("thread2", 40, &sortRandomNum,NULL);
  tid_t thread3 = thread_create("thread3", 30, &outputResult,NULL);
}
